...
public class Digraph<D> {
    ...
    */
    ...
    |--------------------------------------------------------------------------
    */
    public String closeness(int pNumber) { //return is string because of the hashmap doesnt have key => value iterator yet
        int res = 0;
        Queue<GNode<Vertex<D>, Edge>> queue = new Queue<GNode<Vertex<D>, Edge>>();

        GNode<Vertex<D>, Edge> start;
        GNode<Vertex<D>, Edge> v;
        GNode<Edge, Vertex<D>> e;
        int[] reach = new int[vertexes];

        if(vertexes == 0 || edges == 0) return null;

        for (v = head; v != null; v = v.next) v.visited = false;

        if((v = head.outList(pNumber)) == null) return null; //vertex to start
        start = v;
        ...
        while (!queue.isEmpty()) {
            ...
            for(e = v.adj; e != null; e = e.next) {
                if(e.number != pNumber && !e.adj.visited) { //vertex to visit that is not the start one
                    e.adj.visited = true;
                    reach[e.adj.number - 1] = reach[v.number-1] + 1; //sum distance, -1 to the index because array start's at 0
                    queue.enqueue(e.adj);
                }
            }
        }
        ...
        return start.elem.data + " -> " + res / (double)connectedVertexes;
    }
    ...
    */
    ...
    private boolean hasVertex(int number) {
        return head != null && head.outList(number) != null;
    }
