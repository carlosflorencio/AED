...
public class Tree<E> {//TODO: TEST THIS !!!!
	...
	public void insert(E pValue){
		...
		if (root == null){ root = new Node<E>(pValue); return;}
		...
		while(node != null){
			...
			else return; // repetidos na são inseridos 
	...
	public E delete (E pValue){
		...
		while(delnode != null && cmp.compare(delnode, pValue)!= 0){
			// travessia descendente ate encontrar o elemento a remover
			prev = delnode;
			if(cmp.compare(delnode, pValue)>0) delnode= delnode.left;
	...
	public void balance(){
		...
		listInOrder(root,list,0); // preenchê-la com a travessia em ordem
	...
	public String traversal (int pTrav){
		...
		switch(pTrav){
		case PRE_ORDER : str = "pre order :";
						 str = preOrderRec(root,str);
						 break;
		case IN_ORDER : str = "em ordem :";
		 				str = inOrderRec(root,str);
		 break;
		case POST_ORDER : str = "pre order :";
